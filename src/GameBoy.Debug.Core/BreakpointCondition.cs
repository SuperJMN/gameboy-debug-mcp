...
public interface IBreakpointConditionContext
{
...
public sealed class BreakpointCondition
{
    ...
    public DebugResult<bool> Evaluate(IBreakpointConditionContext context)
    {
    ...
    private static bool TryParseLeftOperand(string text, out LeftOperand operand, out string? errorMessage)
    {
        ...
        if (text[0] == '[' || text[^1] == ']')
        {
            ...
            {
                errorMessage = "Memory operands must use [addr] or [16-bit-register].";
                return false;
            }
            ...
            errorMessage = "Memory operands must dereference a decimal or 0x-prefixed address, or AF, BC, DE, HL, SP, or PC.";
            return false;
        }
    ...
    private static bool IsByteRegister(string name) => name.ToUpperInvariant() is "A" or "B" or "C" or "D" or "E" or "F" or "H" or "L";

    private static bool IsWordRegister(string name) => name.ToUpperInvariant() is "AF" or "BC" or "DE" or "HL" or "SP" or "PC";
    ...
    private readonly record struct LeftOperand(LeftOperandKind Kind, string? RegisterName, ushort Address)
    {
        ...
        public DebugResult<int> Read(IBreakpointConditionContext context)
        {
            return Kind switch
            {
        ...
        private DebugResult<int> ReadMemoryAtRegister(IBreakpointConditionContext context)
        {
        ...
        private static DebugResult<int> ReadMemory(IBreakpointConditionContext context, ushort address)
        {
        ...
        private static DebugResult<int> ReadRegister(CpuRegisters registers, string name)
        {
