...
/// <summary>
/// Thread-safe decorator that serializes access to an inner <see cref="IGameBoyDebugSession"/>.
/// The emulator core is single-threaded stateful, while an MCP server may dispatch tool calls
/// concurrently, so every operation is guarded by a single re-entrant lock.
/// </summary>
public sealed class SynchronizedGameBoyDebugSession(IGameBoyDebugSession inner) : IGameBoyDebugSession, IDisposable
{
    ...
    {
        lock (gate) { return inner.SetJoypad(pressedButtons); }
    }
